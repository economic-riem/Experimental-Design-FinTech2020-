...
recent_trd = data.groupby('id').agg({'day': 'max'})
recent_trd['recent_trd'] = recent_trd['day']
del recent_trd['day']
...
for col in cols:
    ...
    for c_v in col_value:
        print(col, c_v)
        data_tmp = data[data[col] == c_v]
        names[col+str(c_v)+'_recent_trd'] = data_tmp.groupby('id').agg({'day': 'max'})
        recent_trd[col + str(c_v) + '_recent_trd'] = 60 - names[col+str(c_v)+'_recent_trd']['day']
        recent_trd['Is_recent_'+col+str(c_v)+'_mean'] = (recent_trd[col+str(c_v)+'_recent_trd'] > recent_trd[col+str(c_v)+'_recent_trd'].mean()) * 1
        fea = pd.merge(fea, recent_trd, how='left', on='id')
print('傻傻居3')
